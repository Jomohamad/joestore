...
export default function GameDetails() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-[60vh] p-4">
        <motion.div 
          ...
          className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8 max-w-md w-full text-center shadow-2xl"
        >
          <div className="w-16 h-16 md:w-20 md:h-20 bg-emerald-500/20 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
          ...
          <div className="bg-zinc-950 rounded-lg p-4 mb-6 md:mb-8 text-left border border-zinc-800/50">
          ...
          <button 
            onClick={() => navigate('/')}
            className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-medium transition-colors text-sm md:text-base"
          >
            Continue Shopping
          </button>
        </motion.div>
  ...
  return (
    <div className="flex-1 bg-zinc-950 pb-20 md:pb-24">
      {/* Game Header */}
      <div className="relative h-48 md:h-64 lg:h-80 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full z-20 pb-6 md:pb-8">
          <div className="container mx-auto px-4 flex items-end gap-4 md:gap-6">
            <div className="w-20 h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 rounded-xl md:rounded-2xl overflow-hidden border-2 md:border-4 border-zinc-900 shadow-2xl shrink-0 bg-zinc-800">
              <img 
                src={game.image_url} 
                alt={game.name} 
            ...
            <div className="mb-1 md:mb-2">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-1 md:mb-2">{game.name}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 md:space-y-6">
            ...
            <section className={cn(
              "bg-zinc-900 border rounded-2xl p-5 md:p-6 shadow-lg transition-colors duration-300",
              validatedPlayerName ? "border-emerald-500/30" : "border-zinc-800"
            )}>
              <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                <div className={cn(
                  ...
                  validatedPlayerName ? "bg-emerald-500 text-white" : "bg-emerald-500/20 text-emerald-500"
                )}>
              ...
              <div className="relative">
                <div className="flex flex-col sm:flex-row gap-3">
                  <input 
                    ...
                    className={cn(
                      "flex-1 bg-zinc-950 border rounded-xl px-4 py-2.5 md:py-3 text-sm md:text-base text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all font-mono",
                      validationError ? "border-red-500/50 focus:border-red-500" : "border-zinc-800 focus:border-emerald-500",
                      validatedPlayerName ? "border-emerald-500/50" : ""
                    )}
                  />
                  <button
                    ...
                    className={cn(
                      "px-6 py-2.5 md:py-3 rounded-xl font-medium transition-all flex items-center justify-center gap-2 min-w-[120px] text-sm md:text-base",
                      validatedPlayerName 
                ...
                {validatedPlayerName && (
                  <motion.div 
                    ...
                    className="flex items-center gap-2 text-emerald-400 text-xs md:text-sm mt-3 bg-emerald-500/10 p-2.5 md:p-3 rounded-lg border border-emerald-500/20"
                  >
                ...
                {!validationError && !validatedPlayerName && (
                  <p className="text-[11px] md:text-xs text-zinc-500 mt-2 md:mt-3">
            ...
            <section className={cn(
              "bg-zinc-900 border border-zinc-800 rounded-2xl p-5 md:p-6 shadow-lg transition-opacity duration-300",
              !validatedPlayerName && "opacity-50 pointer-events-none"
            )}>
              <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center font-bold text-sm md:text-base">2</div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
                {packages.map((pkg) => (
                  ...
                  >
                    {selectedPackage?.id === pkg.id && (
                      <div className="absolute top-0 right-0 w-0 h-0 border-t-[20px] md:border-t-[24px] border-r-[20px] md:border-r-[24px] border-t-emerald-500 border-r-transparent">
                        <CheckCircle2 className="absolute -top-[18px] md:-top-[22px] right-[2px] w-2.5 h-2.5 md:w-3 md:h-3 text-white" />
                      </div>
                    )}
                    
                    <div className="flex items-center gap-1.5 md:gap-2 mb-1.5 md:mb-2">
                      <span className="text-lg md:text-xl font-bold text-white">{pkg.amount}</span>
                      <span className="text-xs md:text-sm font-medium text-zinc-400">{game.currency_name}</span>
                    </div>
                    
                    {pkg.bonus > 0 && (
                      <div className="text-[10px] md:text-xs font-medium text-emerald-400 mb-2 md:mb-3 bg-emerald-500/10 inline-block px-1.5 md:px-2 py-0.5 rounded">
                        +{pkg.bonus} Bonus
                      </div>
                    )}
            ...
            <section className={cn(
              ...
              <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center font-bold text-sm md:text-base">3</div>
                ...
                {['Credit Card', 'PayPal', 'Crypto'].map((method) => (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      {selectedPayment === method && <div className="w-2 h-2 md:w-2.5 md:h-2.5 bg-emerald-500 rounded-full" />}
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-20 md:top-24 bg-zinc-900 border border-zinc-800 rounded-2xl p-5 md:p-6 shadow-xl">
              <h3 className="text-base md:text-lg font-bold text-white mb-4 md:mb-6 pb-3 md:pb-4 border-b border-zinc-800">Order Summary</h3>
              ...
              <div className="space-y-3 md:space-y-4 mb-4 md:mb-6">
                ...
                {validatedPlayerName && (
                  <div className="flex justify-between text-xs md:text-sm">
                    ...
                    <span className="text-emerald-400 font-medium truncate max-w-[120px] md:max-w-[150px]">{validatedPlayerName}</span>
                  </div>
                )}
              ...
              <div className="pt-3 md:pt-4 border-t border-zinc-800 mb-6 md:mb-8">
                <div className="flex justify-between items-end">
                  <span className="text-xs md:text-sm text-zinc-400 font-medium">Total Price</span>
              ...
              <p className="text-[10px] md:text-xs text-center text-zinc-500 mt-3 md:mt-4">
